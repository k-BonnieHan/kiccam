...
public class BiddingMgr {
	...
	public void insertOrder(OrderBean order) {
		try {
			...
			String sql = "insert into stock_order(product_no,quantity,price, sdate,state,id) values(?,?,?,now(),?,?)";
	...
	public ArrayList<OrderDto> getOrder(String id){
		...
		try {
			conn = ds.getConnection();
			String sql ="select * from stock_order where id=? order by no desc";
			pstmt = conn.prepareStatement(sql);
	...
	public OrderDto getOrderDetail(String no) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from stock_order where no=?";
			pstmt = conn.prepareStatement(sql);
	...
	public boolean updateOrder(String no, String state) {
		...
		try {
			conn = ds.getConnection();
			String sql = "update stock_order set state=? where no=?";
			pstmt = conn.prepareStatement(sql);
	...
	public boolean deleteOrder(String no) {
		...
		try {
			conn = ds.getConnection();
			String sql = "delete from stock_order where no=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, no);
			if(pstmt.executeUpdate() > 0) b = true;
			
			//상품의 재고 수량을 원상 복구 작업 필요... 생략. 
		} catch (Exception e) {
